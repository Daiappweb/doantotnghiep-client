...
import AddImportForm from "@/components/dialog/AddImport";
  ...
  return (
    <Box>
      <DialogApp openPopup={open} handleClosePopup={handleClosePopup}>
        <AddImportForm onClosePopup={handleClosePopup}></AddImportForm>
      </DialogApp>
      <Toolbar
        sx={{
          pl: { sm: 2 },
      ...
      >
        ...
        <IconButton>
          <IoIosAddCircle size={24} onClick={() => handleAddImport()} />
        </IconButton>
        {numSelected > 0 ? (
...
export default function ImportReceipt() {
  ...
  function createData(id, name, code, createddate, totalamount) {
    return {
      id,
      name,
      code,
  ...
  const loadImports = async () => {
    try {
      const imports = await authService.getAllDetails();
      console.log("orders ==== ", imports);
      const rowRes = [];
